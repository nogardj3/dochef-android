...
//    TODO 이거 쳐내고 signupdactivity에서 응답받으면 nickname 뜨는걸로 바꾸기
    ...
    public void signUp() {
        ...
        if (nickname.length() < 1) {
            DoChef.getAppInstance().showToast("닉네임의 길이가 너무 짧습니다. 1자 이상 입력해주세요");
        } else if (!nickname.matches("[0-9|a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힝]*")) {
            DoChef.getAppInstance().showToast("사용할 수 없는 닉네임입니다. 숫자, 알파벳 대소문자, 한글만 사용가능합니다.");
        } else {
            ...
            signUpService.SignupCall(AccessToken, nickname)
                    .enqueue(new BasicCallback<JsonObject>(SignupNicknameActivity.this) {
                        ...
                        public void onResponse(Response<JsonObject> response, int err) {
                            ...
                            switch (err) {
                                case 822:
                                    DoChef.getAppInstance().showToast("이미 존재하는 닉네임입니다.");
                                    break;
                                case 825:
                                    DoChef.getAppInstance().showToast("닉네임의 길이가 너무 짧습니다. 1자 이상 입력해주세요");
                                    break;
                                case 826:
                                    ...
                                    break;
                                case 0:
                                    DoChef.getAppInstance().showToast("회원가입되었습니다.");
